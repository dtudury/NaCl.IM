...
exports.respond_octet_stream = function (res, obj, code) {
    res.writeHead(code || 200, {'Content-Type': 'application/octet_stream'});
    res.end(obj);
};

exports.collect_and_call = function (http_socket, callback) {
    var args = [].slice.call(arguments, 2);
    var buffers = [];
    http_socket.on('data', function (data) {
        buffers.push(data);
    });
    http_socket.on('end', function () {
        callback.apply(null, [null, Buffer.concat(buffers)].concat(args));
    });
    http_socket.on('error', function () {
        callback.apply(null, [new Error("http_socket error"), null].concat(args));
    });
};
